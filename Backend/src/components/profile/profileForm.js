...
export const ProfileForm = ({addUser,errorMsg, errUpdateInfo,user}) => {
    ...
    let catchPhrase = user.catchPhrase;
                    ...
                    <input id="update" className="btn" type="submit" value="Update"
                           ...
                           {event.preventDefault();
                               validateInfo(addUser,errorMsg,pw1,pw2,name,
                                   email,phone,zipcode,birthday,user,imgsrc,follow,catchPhrase)}
                           }/>
...
const validateInfo=(addUser,errorMsg,Pw1,Pw2,Name,Email,Phone,Zipcode,Birthday,user,imgsrc,follow,catchPhrase)=>{
    var flag=true;
    var str="";
    errorMsg("")
    ...
    if(flag){
        if (unchanged(user,Name,Email,Phone,Zipcode,Pw1,Pw2,follow,catchPhrase))
            errorMsg("You didn't change anything. ")
        else{
            updateuser(addUser,errorMsg,Pw1,Pw2,Name,Email,Phone,Zipcode,Birthday,user,imgsrc,follow,catchPhrase);
...
const updateuser=(addUser,errorMsg,Pw1,Pw2,Name,Email,Phone,Zipcode,Birthday,user,imgsrc,follow,catchPhrase)=>{
    let myName;
    let myEmail;
    let myPhone;
    let myZipcode;
    ...
    (dispatch) => {
        return {
            addUser: (user,follower)=>
                dispatch({type: 'PROFILE_CHANGE',user,follower}),
